function [ max_prods ] = maxproduct( A,n)
    ...
    [mpr,maxpR]=checkRows(A,n);  if mpr>-Inf maxpR=sortrows(maxpR,1); end;
    [mpd,maxpD]=checkDiags(A,n); if mpd>-Inf maxpD=sortrows(maxpD,1); end;
    [mpc,maxpC]=checkRows(A',n);if mpc>-Inf maxpC=sortrows(fliplr(maxpC),1);end;
    [mpod,maxpOD]=checkDiags(fliplr(A),n); if mpod >-Inf maxpOD=sortrows(maxpOD,1);end;
...
function [mp,max_prods]=checkRows(A,n)
    if n > size(A,2)
        mp=-Inf;
        max_prods=[];
        return
...
function [mp,newxp]=checkDiags(A,n)
   
    d=diags(A,n);
    [mp,max_prods]=checkRows(d,n) ; 
